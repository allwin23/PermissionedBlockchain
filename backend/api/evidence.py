...
def submit_evidence():
    try:
        ...
        requestor = wallet.load_identity(session['username'], client=fabric_client.client)
        tx_id = fabric_client.submit_transaction(requestor, 'SubmitEvidence', args)
...
def get_evidence(evidence_id):
    try:
        requestor = wallet.load_identity(session['username'], client=fabric_client.client)
        response = fabric_client.query_transaction(requestor, 'QueryEvidence', [evidence_id])
...
def get_all_evidence():
    try:
        requestor = wallet.load_identity(session['username'], client=fabric_client.client)
        response = fabric_client.query_transaction(requestor, 'QueryAllEvidence', [])
        return jsonify({'status': 'success', 'data': json.loads(decode_response(response))}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400
...
def update_status(evidence_id):
    ...
    try:
        requestor = wallet.load_identity(session['username'], client=fabric_client.client)
        tx_id = fabric_client.submit_transaction(requestor, 'UpdateEvidenceStatus', [evidence_id, data['status']])
...
def transfer_custody(evidence_id):
    ...
    try:
        ...
        tx_id = fabric_client.submit_transaction(requestor, 'TransferCustody', [evidence_id, data['newOwningOrg'], reason])
        return jsonify({'status': 'success', 'tx_id': tx_id}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400
